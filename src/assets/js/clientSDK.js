    ...
    return new Promise((resolve, reject) => {
        ...
        if (!checkClient()) {
          ...
          if(queryParam && queryParam.store_id && queryParam.store_name){
            ...
            sdk_fn(insight.storage.get,'current_dashboard')
            .then(data => {
                let objKey = JSON.parse(data)
                let generalKey ='store_info'
                if (objKey.name == 'multi_brands_dashboard') {
                    generalKey='brands_info'
                }else if(objKey.name == 'staff_dashboard'){
                    generalKey='staff_info'
                }
                // alert(generalKey)

                insight.storage.get(generalKey)
                .then(responseData => {
...
const getUserInf = function () {
    return new Promise ((resolve, reject) => {
        sdk_fn(insight.open.getUserInfo)
        .then( data => {
            resolve(data);
        })
        .catch( error => {
